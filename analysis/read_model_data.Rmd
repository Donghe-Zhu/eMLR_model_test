...
```{r read_landsea_mask, eval=FALSE}
...
```{r assign_region_labels, eval=FALSE}
...
```{r assign_basin_labels, eval=FALSE}
...
```{r basin_masks_WOA18_map, fig.asp=0.6, eval=FALSE}
...
```{r create_section_global, eval=FALSE}
...
```{r section_global_map, fig.asp=0.6, eval=FALSE}
...
### Read netcdf
Below I think there are two ways to achieve the regriding. I prefer the first one, although there are more errors here than the second simpler one.
```{r read_file_and_regrid_data, eval=FALSE}

# method 1

read_regrid <- function(var) {
  
#### here need to set nc_data_tibble_regrided as a empty table first, but I am not sure how to set it.
...
reture(nc_data_tibble_regrided)

}

nc_data_tibble_regrided <- read_regrid(c("DIC","ALK","O2","AOU","NO3","PO4","SALT","TEMP","SiO3"))
...
reture(nc_data_tibble)

}

nc_data_tibble_regrid_DIC <- read_regrid("DIC")
nc_data_tibble_regrid_ALK <- read_regrid("ALK")
nc_data_tibble_regrid_O2 <- read_regrid("O2")
nc_data_tibble_regrid_AOU <- read_regrid("AOU")
nc_data_tibble_regrid_NO3 <- read_regrid("NO3")
nc_data_tibble_regrid_PO4 <- read_regrid("PO4")
nc_data_tibble_regrid_SALT <- read_regrid("SALT")
nc_data_tibble_regrid_TEMP <- read_regrid("TEMP")
nc_data_tibble_regrid_SiO3 <- read_regrid("SiO3")

nc_data_tibble_regrided <- full_join(c(nc_data_tibble_regrid_DIC,nc_data_tibble_regrid_ALK,nc_data_tibble_regrid_O2,nc_data_tibble_regrid_AOU,nc_data_tibble_regrid_NO3,nc_data_tibble_regrid_PO4,nc_data_tibble_regrid_SALT,nc_data_tibble_regrid_TEMP,nc_data_tibble_regrid_SiO3))


####################################################
  
# filter surface data
nc_data_tibble_surface <- nc_data_tibble_regrided %>%
  filter(depth == 5)
...
```{r define section_global function, eval=FALSE}
...
```{r define_section_regional_function, eval=FALSE}
...
```{r Zonal_section_DIC, eval=FALSE}
...
```{r Zonal_section_ALK, eval=FALSE}
...
```{r Zonal_section_AOU, eval=FALSE}
...
```{r Zonal_section_NO3, eval=FALSE}
...
```{r Zonal_section_PO4, eval=FALSE}
...
```{r Zonal_section_SALT, eval=FALSE}
...
```{r Zonal_section_TEMP, eval=FALSE}
...
```{r Zonal_section_SiO3, eval=FALSE}
